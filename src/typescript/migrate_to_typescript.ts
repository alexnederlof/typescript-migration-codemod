  ...
  traverse.default(file, {
    ...
    TypeAnnotation(path) {
      ...
      replaceWith(
        path,
        t.tsTypeAnnotation(
          migrateType(reporter, filePath, path.node.typeAnnotation)
        )
      );
    },
    ...
    OpaqueType(path) {
      if (path.node.supertype)
        throw new Error(
          "Opaque types with a supertype are currently not supported."
        );
    ...
    FunctionDeclaration: {
      enter(path) {
        ...
        awaitPromises.push(
          annotateParamsWithFlowTypeAtPos(reporter, filePath, path.node.params)
        );
        ...
        if (isTestFile) {
          for (const param of path.node.params) {
            if (!(param as any).typeAnnotation) {
              (param as any).typeAnnotation = t.tsTypeAnnotation(
                t.tsAnyKeyword()
              );
            }
          }
    ...
    ClassMethod(path) {
      // Add Flow’s inferred type for all unannotated function parameters...
      const recastParams = path.node.params as t.FunctionDeclaration["params"];
      awaitPromises.push(
        annotateParamsWithFlowTypeAtPos(
          reporter,
          filePath,
          recastParams as t.FunctionDeclaration["params"]
        )
      );
    },
  ...
  switch (flowType.type) {
    ...
    case "ArrayTypeAnnotation":
      return t.tsArrayType(
        migrateType(reporter, filePath, flowType.elementType)
      );
      ...
      const params = flowType.params.map<t.Identifier | t.RestElement>(
        (flowParam, i) => {
          const tsParam = tsIdentifier(
            ...
            flowParam.optional,
            t.tsTypeAnnotation(
              migrateType(reporter, filePath, flowParam.typeAnnotation)
            )
          );
      ...
      for (const flowMember of flowMembers) {
        if (flowMember.type === "ObjectTypeSpreadProperty") {
          ...
          const tsArgument = t.tsParenthesizedType(
            migrateType(reporter, filePath, flowMember.argument)
          );
    ...
    const tsTypeParameter = t.tsTypeParameter(
      flowTypeParameter.bound
        ? migrateType(
            reporter,
            filePath,
            flowTypeParameter.bound.typeAnnotation
          )
        : null,
...
function actuallyMigrateObjectMember(
  ...
  switch (flowMember.type) {
    ...
    default: {
      ...
      throw new Error(
        `Unrecognized AST node: ${JSON.stringify(never["type"])}`
      );
    }
  }
}
  ...
  if (expression.type === "ObjectExpression") {
    for (const property of expression.properties) {
      if (property.type === "ObjectMethod") {
        ...
        if (
          property.computed &&
          !isComplexLiteral(property.key as t.Expression)
        ) {
          return false;
        }
        if (
