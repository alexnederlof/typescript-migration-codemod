...
import isBabelInteropDefaultImportTheSame from './babel_interop_default_import.js';
import MigrationReporter from './migration_reporter.js';
import shouldMigrateFilePath from './should_migrate_file_path.js';
    ...
    Identifier(path) {
      ...
      if (path.node.name === "require") {
        if (path.parent.type !== "CallExpression") {
          ...
          if (path.parent.arguments.length !== 1) {
            throw new Error(
              `Expected require() calls to only have one argument.`
            );
          }
          ...
          if (moduleId.type !== "StringLiteral") {
            ...
            let modulePath;
            try {
              modulePath = require.resolve(moduleId.value, {
                paths: [dirname(filePath)],
              });
            } catch (error) {
              reporter.cannotFindModule(
                moduleId.value,
                filePath,
                moduleId.loc!
              );
            ...
            if (modulePath !== null) {
              ...
              if (callPath.node.type !== "CallExpression") {
                throw new Error(
                  "Expected above conditions to prove this is a call expression."
                );
              }
              ...
              if (shouldMigrateFilePath(filePath)) {
                ...
                if (shouldMigrateFilePath(modulePath)) {
                  replaceWith(
                    callPath,
                    t.memberExpression(callPath.node, t.identifier("default"))
                  );
                }
              }
            }
      ...
      if (path.node.name === "module") {
        ...
        if (
          ...
          if (shouldMigrateFilePath(filePath)) {
            const assignmentPath = path.parentPath.parentPath; // `Identifier` // `MemberExpression` // `AssignmentExpression`

            if (assignmentPath.node.type !== "AssignmentExpression") {
              throw new Error(
                `Expected above conditions to prove this is an assignment expression.`
              );
            }

            const statementPath = assignmentPath.parentPath; // `AssignmentExpresison` // `ExpressionStatement`
...
function migrateRequireToImport({
  ...
  if (shouldMigrateFilePath(modulePath)) {
    ...
    const sameBabelInteropDefaultImport =
      isBabelInteropDefaultImportTheSame(modulePath);
  ...
  const moduleWithNamedExport = allowlists.modulesWithNamedExports.get(
    moduleId.value
  );
  ...
  if (
    ...
    if (moduleWithNamedExport === undefined) {
      ...
      if (
        importPattern.type === "Identifier" &&
        !importPattern.typeAnnotation
      ) {
        replaceWith(
          callPath.parentPath.parentPath,
          t.importDeclaration(
        ...
        importPattern.type === "ObjectPattern" &&
        defaultImport === false
      ) {
        ...
        replaceWith(
          callPath.parentPath.parentPath,
          t.importDeclaration(importSpecifiers, moduleId)
        );
      } else {
        ...
        const moduleName = camelCase(
          basename(moduleId.value, extname(moduleId.value))
        );
        ...
        (destructVariableDeclaration as any).recastDisableMultilineSpacing =
          true;
      ...
      if (importPattern.type === "Identifier") {
        ...
        switch (moduleWithNamedExport.defaultImport) {
          ...
          case "none":
            throw new Error(
              `Did not expect '${moduleId.value}' to have a default import.`
            );
          default: {
        ...
        //     throw new Error(
        //       `Unexpected import '${property.key.name}' from '${moduleId.value}'`
        //     );
...
function expressionHasSideEffects(expression: t.Expression): boolean {
  ...
  if (expression.type === "MemberExpression") {
    ...
    let ex: t.Expression = expression;
    throw new Error("Fix me")
    // return (
    //   expressionHasSideEffects(ex.object) ||
    //   expressionHasSideEffects(ex.property)
    // );
  }
  return true;
}
 ...
 * https://github.com/benjamn/recast/issues/572
 */
function replaceWithMultipleCopyingComments<T extends t.Node>(
  path: NodePath<T>,
  nodes: Array<T>
) {
